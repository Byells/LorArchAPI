...
public static class DefeitoMotoEndpoints
{
    ...
    private static async Task<IResult> GetDefeitosMoto(ApplicationDbContext db)
    {
        return Results.Ok(await db.DefeitoMotos.ToListAsync());
    }

    private static async Task<IResult> GetDefeitoMotoById(int id, ApplicationDbContext db)
    {
        return await db.DefeitoMotos.FindAsync(id) is DefeitoMoto dm
            ? Results.Ok(dm)
            : Results.NotFound();
    }
